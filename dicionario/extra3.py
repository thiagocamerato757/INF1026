...
def qualNota(alunos: dict, matricula: str, disciplina: str) -> float:
    if matricula not in alunos:
        return -1
    else:
        return alunos[matricula].get(disciplina, -2)
...
def criaDicBonsDeUmaDisc(alunos: dict, disciplina: str) -> dict:
    result_dict: dict = {}
    for aluno, dados in alunos.items():
        nota: float = dados.get(disciplina, 0)
        if nota > 7.0:
            result_dict[aluno] = nota
    return result_dict
...
#            VALOR: nota do aluno na disciplina
def criaDicPorDisc(alunos: dict) -> dict:
    result_dict: dict = {}
    for aluno, dados in alunos.items():
        for disciplina, nota in dados.items():
            result_dict[disciplina] = result_dict.get(disciplina, {})
            result_dict[disciplina][aluno] = result_dict[disciplina].get(aluno, 0) + nota
    return result_dict
